...
echo "== Verun Solana MVP live smoke =="
echo "Base: $BASE_URL"
...
jq -r '.balance.sol' /tmp/verun_funding.json

echo
printf "[4/5] config check ...... "
curl -sf "$BASE_URL/api/config-check" >/tmp/verun_config.json
jq -r '.checks.secret_present, .checks.secret_valid, .checks.rpc_reachable' /tmp/verun_config.json
